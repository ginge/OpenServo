...
#pragma pack(push, 1)
	...
	struct SUndoRecord
	{
		...
		TCHAR	m_cDescription[ 32 ];				// Action Description
...
#pragma pack(pop)
	...
	//	[JRT]: Support For Descriptions On Undo/Redo Actions...
	//
	void AddUndoRecord(BOOL bInsert, const CPoint &ptStartPos, const CPoint &ptEndPos, LPCTSTR pszText, LPCTSTR cpDesc = NULL );
...
public:
	...
	BOOL InsertText(CCrystalTextView *pSource, int nLine, int nPos, LPCTSTR pszText, int &nEndLine, int &nEndChar, LPCTSTR cpDesc = NULL );
	BOOL DeleteText(CCrystalTextView *pSource, int nStartLine, int nStartPos, int nEndLine, int nEndPos, LPCTSTR cpDesc = NULL );

	//
	//	[JRT]: Support For Descriptions On Undo/Redo Actions...
	//
	BOOL CanUndo( PTSTR cpDesc = NULL );
	BOOL CanRedo( PTSTR cpDesc = NULL );
