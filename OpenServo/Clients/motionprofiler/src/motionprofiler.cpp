...
motionprofiler::motionprofiler()
    : QMainWindow( 0, "motionprofiler", WDestructiveClose )
{
...
void motionprofiler::newDoc()
{
    motionprofiler *ed = new motionprofiler;
    ed->setCaption(tr("Qt Example - Application"));
    ed->show();
}

void motionprofiler::choose()
{
...
void motionprofiler::load( const QString &fileName )
{
    QFile f( fileName );
...
void motionprofiler::closeEvent( QCloseEvent* ce )
{
    if ( !e->isModified() ) {
	ce->accept();
	return;
